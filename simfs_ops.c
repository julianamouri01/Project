...
/* loads the directory entries from the start of the file */
void
...
/* saves the directory entries back to the start of the file */
void
...
/* loads the fnode array which sits right after the fentries */
void
...
/* saves the fnode array back to its position in the file */
void
...
/* figures out where data blocks start - metadata takes up the front
 * of the file so we round up to the next block boundary */
long
...
/* returns the byte offset of a given data block number in the file */
long
...
/* reads one full block from the data area into buf */
void
...
/* writes one full block from buf into the data area */
void
...
createfile(char *fsname, char *filename)
{
    ...
    /* scan for duplicates and grab the first open slot */
    int slot = -1;
...
/* removes a file and frees all its blocks */
void
deletefile(char *fsname, char *filename)
{
    ...
    /* walk the chain, zero each block, and mark each fnode as free */
...
/* reads length bytes starting at offset start and prints to stdout */
void
readfile(char *fsname, char *filename, int start, int length)
{
    ...
    /* walk the fnode chain block by block, output only the bytes we need */
...
/* writes length bytes from stdin into the file starting at offset start */
void
writefile(char *fsname, char *filename, int start, int length)
{
    ...
    /* check free space before touching anything - all or nothing */
    if (to_alloc > 0) {
        int free_count = 0;
    ...
    /* read all input first so we dont partially write on a read failure */
    ...
    /* find the tail of the existing fnode chain */
    short tail = -1;
    short cur = files[slot].firstblock;
    ...
    /* allocate new blocks and link them onto the chain */
    ...
    /* walk the chain and write data into the blocks it covers */
    int file_pos = 0;
    ...
    /* update the file size in the fentry */
